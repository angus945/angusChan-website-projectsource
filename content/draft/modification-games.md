...
一年前就接觸的資料驅動，終於有時間深入了。這篇是我實驗並整理出的筆記，模組的原理與透過 IO, XML 與 Lua 在 Unity 中實現的方法。
...
=======
### 什麼是遊戲模組 -
>>>>>>> b6fc3bd412208c05e79f405715f094de247272dc

遊戲模組 (game mod) 是一種能讓玩家對遊戲內容進行擴展的手段，透過開發者預定好的框架，讓玩家在一定範圍內自由修改遊戲，改變角色外觀、添加道具與敵人，甚至改變遊戲規則，Minecraft 就是相當經典的案例，玩家能透過材質、光影包來增添視覺效果，或安裝模組讓遊戲更加科幻或奇幻。

模組除了提升玩家 也能對遊戲開發過程起到幫助
...
"Data Driven Design" 如果嘗試查詢模組相關資訊的話，大多資訊都會指向這個關鍵字，資料驅動。簡單來說，資料驅動是一種程式設計法，其核心思想在於透過簡單的資料結構定義出一系列"邏輯"或者"行為"，例如字串和整數，並使用資料為底驅動整個遊戲，而不是透過程式將設計寫死
>>>>>>> b6fc3bd412208c05e79f405715f094de247272dc

如果你有嘗試查詢過模組架構的實現方法，相信大部分的資料都會指向一個關鍵字：資料驅動 (Data Driven)，這是一種透過簡單的資料結構定義出一系列行為與邏輯的程式設計手法，使用字串與整數建構並驅動遊戲運作，而不是透過程式將設計寫死。

這種作法除了能大幅提升修改彈性與降低設計門檻，還間接提供了玩家擴展內容的手段。但要真正深入的話可能要花不少時間，所以這裡就將與模組關聯的內容整理成三大重點，提供各位進一步研究的參考。
